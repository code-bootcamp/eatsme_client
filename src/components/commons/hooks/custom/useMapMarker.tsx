...
export const useMapMarker = (): IUseMapMarker => {
  ...
  const searchMapMarker = (data: any): void => {
    ...
    const restaurantNames = path.info.map((el: any) => el.restaurantName);
    for (let i = 0; i < data.length; i++) {
      if (restaurantNames.includes(data[i].name) === true) continue;
      // 마커 위치 잡기
      ...
      TMarker.addListener("click", () => {
        onInfoWindow(position, map, data[i], -1);
      });
  ...
  const pickMapMarker = (): void => {
    ...
    for (let i = 0; i < path.info.length; i++) {
      ...
      // 마커 클릭 이벤트 작성 (PC,모바일)
      TMarker.addListener("click", () => {
        onInfoWindow(position, map, path.info[i], i);
      });
      TMarker.addListener("touchstart", () => {
        onInfoWindow(position, map, path.info[i], i);
      });
